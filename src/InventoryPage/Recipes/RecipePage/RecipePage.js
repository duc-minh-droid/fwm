...
function RecipePage() {
    ...
    function filterHTMLstring(string) {
        if (string) {
            console.log(string)
            const regex = /<\s*href=/g;
            const fixedString = string.replace(regex, '<a href=');
            const sentences = fixedString.split('. '); // Assuming sentences are separated by '. '
            let scoreSentences = sentences.filter(sentence => !sentence.toLowerCase().includes('score'));
            scoreSentences = scoreSentences.join('. '); // Joining the filtered sentences back with '. '
            return scoreSentences + ".";
        }
    }
    ...
    <div style={{backgroundColor: '#91BD96'}}>
        {recipeData && <div style={{ fontFamily: 'Arial, sans-serif', margin: '0 auto', padding: '40px 100px 40px 100px' }}>
                ...
                <div style={{ backgroundColor: '#f5f5f5', padding: '15px', borderRadius: '8px' }}>
                    ...
                    <h3>Instructions:</h3>
                    <p>Find more details <a href={recipeData.spoonacularSourceUrl}>here</a>.</p>
                </div>
            </div>}
    </div>
  )
